...
for img_path in img_paths:
    ...
    mat_path = img_path.replace('.bmp', '_detection.mat')
    mat = io.loadmat(mat_path)
    # print(mat_path)
    mat_save_path = './UW/gt_file/' + mat_path.split('/img')[1] + '.mat'
    print(mat_save_path)
    shutil.copyfile(mat_path, mat_save_path)
    ...
    # 前面84个放进训练集，后面放进测试集
    path = 'train' if k<=70 else 'test'
    
    h5_path = img_path.replace('.bmp', '.h5').replace('UW_dataset/Detection', path).replace(img_path.split('/')[-2],'location_map', 1)
...
train_files = os.listdir('./UW/train/images')
test_files = os.listdir('./UW/test/images')
