...
class VGG16_FPN(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.de_pred = nn.Sequential(
            ...
            nn.Conv2d(1280,128,kernel_size=3,stride=1,padding=1,bias=True),
		    nn.BatchNorm2d(128),
    ...
    def forward(self, x):
        ...
        for x3_in in x3:
            ...
            # print(x3_in.shape)        # [384, 8, 8]
            # Cos_adj = self.Cos_Adj(x3_in)
            # # print(Cos_adj.shape, Cos_adj[0][:20])        # ([64, 64])

            # Final_adj = Dist_adj + Cos_adj
        ...
        # print(output[0][0][:10], f[2][0][0][0][:10])
        x2_weight = output.view(f[2].shape[0], f[2].shape[1], f[2].shape[2], -1)
        normalized_weight = F.normalize(x2_weight, p=2, dim=1)

        x2 = x3 * normalized_weight
        # x2 = x3 + x2_weight
        # print(x2.shape)
