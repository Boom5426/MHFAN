...
class HighResolutionNet_CDC(nn.Module):

    def __init__(self, config, **kwargs):
        extra = config.MODEL.EXTRA
        super(HighResolutionNet_CDC, self).__init__()

        # stem net
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.conv1_1 = MDC(nIn=64, nOut=64)
        # self.conv1_1 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1,
        #                        bias=False)
        self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
...
def get_seg_model(train=False):
    ...
    # # model = HighResolutionNet_CDC(hr_config)
    # model = HighResolutionNet(hr_config)
    # from Networks.HR_Net.config import cfg

    # # test
    from default import _C as hr_config
