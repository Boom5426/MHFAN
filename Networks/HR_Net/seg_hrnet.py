...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.Conv2d(last_inp_channels,128,kernel_size=3,stride=1,padding=1,bias=True),
		    nn.BatchNorm2d(128),
			nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2),
    ...
    def init_weights(self, pretrained='', train=True):
...
if __name__ == '__main__':
    ...
    net = get_seg_model(train=False).cuda()
    # print(net)
    input = torch.randn(1, 3, 640, 427).cuda()
    # x= net(input)
